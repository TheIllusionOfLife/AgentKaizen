...
def emit(message: str, usage: dict[str, int] | None = None) -> int:
    ...
    sys.stdout.write(json.dumps({"type": "turn.completed", "usage": usage}) + "\n")
    return 0
...
def main(argv: list[str]) -> int:
    ...
    while idx < len(args):
        ...
        if arg in {"--json", "--skip-git-repo-check"}:
            idx += 1
            continue
        if arg in {"--model", "--sandbox", "--profile", "--image"} and idx + 1 < len(
            args
        ):
            idx += 2
            continue
    ...
    if prompt == "Say only: ok":
        return emit(
            "ok", usage={"input_tokens": 2, "output_tokens": 1, "total_tokens": 3}
        )
    ...
    if "what does this repository do?" in prompt_lower:
        if "respond in japanese" in agents_text:
            return emit(
                "このリポジトリはCLIエージェントの挙動をW&B Weaveで測定します。"
            )
        return emit("This repository measures CLI agent behavior with W&B Weave.")
